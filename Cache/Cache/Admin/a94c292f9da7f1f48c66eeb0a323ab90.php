...
<div class="table-list">
    <table width="100%" cellspacing="0">
        <thead>
      		<tr>
			<th width="40">ID</th>
			<th align="left"><?php echo L('user_group'); echo L('name');?></th>
      	...
      	<tbody>
      		<?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$role): $mod = ($k % 2 );++$k;?><tr>
			<td align="center"><?php echo ($role['id']); ?></td>
      		<td><?php echo ($role['name']); ?></td>
      		<td><?php echo ($role['remark']); ?></td>
			<td align="center">
      		<?php if(($role['status']) == "1"): echo L('enable');?>
      		<?php else: ?>
      		<?php echo L('disable'); endif; ?>
      		</td>
      		<td align="center">
			<a href="<?php echo U('Role/edit',array(id=>$role['id']));?>"><?php echo L('edit');?></a> |
			<a href="javascript:confirm_delete('<?php echo U('Role/delete',array(id=>$role['id']));?>')"><?php echo L('delete');?></a> |
      		...
      		<a href="<?php echo U('User/index',array(groupid=>$role['id']));?>"><?php echo L('user_list');?></a>
      		</td>
