...
<div id="header" class="header">
 <div class="logo">
            <!--<span class="logo_cn">网站管理系统</span><span class="logo_en">Manage System</span>-->
            <span class="logo_cn">
        <img src="./Public/Images/logo2.png" style="height:55px;" alt=""/>
        </span>
        </div>
	<div class="nav">&nbsp;&nbsp;&nbsp;&nbsp;<?php echo L('welcome_user'); echo (session('username')); ?>  <i>|</i> [<?php echo ($usergroup); ?>]  <i>|</i> [<a href="<?php echo U('Login/logout');?>" target="_top"><?php echo L('logout');?></a>]  <i>|</i> <a href="<?php echo ($site_url); ?>" target="_blank"><?php echo L('home_index');?></a>  <i>|</i> <a href="javascript:void(0);"  onclick="gocacheurl();" ><?php echo L('UPDATE_CACHE');?></a>
            <i>|</i> <a href="/index.php?g=Bootstrap&a=index" target='_blank'>Boostrap编辑器</a>
            <i>|</i> <a href="./Public/Images/后台使用说明.pdf" target='_blank'>帮助</a>
        </div>
</div>
...
<div id="Main_content">
	...
	<div id="leftMenuBox">
    	<div id="leftMenu">
            <?php if(is_array($menu)): $n = 0; $__LIST__ = $menu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$r): $mod = ($n % 2 );++$n;?><dl id="nav_<?php echo ($r['bnav']['id']); ?>" <?php if($n==2) : ?>class="on"<?php endif;?>>
                    <dt><a href="javascript:void(0);"><span class="iconfont icon-<?php echo ($n); ?>"></span><?php echo ($r['bnav']['name']); ?><i></i></a></dt>
                    <dd <?php if($n==2) : ?>style="display:block;"<?php endif;?>>
                        <ul>
                        <?php if(is_array($r[nav])): $i = 0; $__LIST__ = $r[nav];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><li id="li_<?php echo ($item['id']); ?>"><span  onclick="javascript:gourl('<?php echo ($item['id']); ?>','<?php echo U($item['model'].'/'.$item['action'],$item['data']);?>')"><a href="<?php echo U($item['model'].'/'.$item['action'],$item['data']);?>" class="name" target="main"><i></i><?php echo ($item['name']); ?></a><?php if($r[bnav]['model']=='Category') : ?><a href="<?php echo U($item['model'].'/add',$item['data']);?>" id="<?php echo ($i); ?>" class="add" target="main"><?php echo L(add);?></a><?php endif;?></span></li><?php endforeach; endif; else: echo "" ;endif; ?>
                        </ul>
                    </dd>
