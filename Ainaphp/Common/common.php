 ...
 * 导出数据生成excel
 * $title           表名
 * $fields          数据字段 $fields = array( 'stuNo', 'name', 'class' );
 * $fieldsval       填充第一行内容 $fieldsval = array( '学号', '姓名', '班级');
 * $data            数据$data = [['stuNo' => '20190101','name' => 'student01','class' => '1班'],['stuNo' => '20190102','name' => 'student02','class' => '1班']];
 */
function getExcel($title,$fields,$fieldsval,$data){
//    echo 222;exit;
...
//        $obj = new \PHPExcel();
	$obj = new PHPExcel();
        ...
        // 填充第一行数据
                foreach($fieldsval as $kf=>$vf){
           $obj->getActiveSheet() ->setCellValue($list[$kf] . '1', $vf);
                }
        // 填充第n(n>=2, n∈N*)行数据
foreach($data as $k=>$v){
    foreach($fields as $ii=>$vv){
    $obj->getActiveSheet()->setCellValue($list[$ii] . ($k + 2), $v[$vv]);//将其设置为文本格式
    }
}
        // 设置加粗和左对齐
        foreach ($list as $col) {
...
//$obj->getActiveSheet()->getDefaultColumnDimension()->setWidth(-1);
//$obj->getActiveSheet()->getDefaultRowDimension()->setRowHeight(-1);
